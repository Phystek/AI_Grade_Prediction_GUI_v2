...
class MainApp(ctk.CTk, TkinterDnD.DnDWrapper):
    def __init__(self):
        ...
        self.train_frame_left = ctk.CTkFrame(master=self.train_frame, corner_radius=10, fg_color=LABEL_COLOR, width=250)
    ...
    def store_data_p(self):
        ...
        if self.no_of_datafiles_p == 1:
            ...
            # place dnd image on the canvas
            self.image_p = Image.open("images/predict_ready.png")
            self.original_image_p = self.image_p
            self.update_canvas()
            self.canvas_p.pack(expand=True, fill="both")
    ...
    def remove_data_tree_p(self):
        self.checkboxes_frame_outer_p.grid_forget()
        self.data_tree_p.grid_forget()
